...
//                headerAndContent.css('transform','translate(0,0)');
            ...
            $('[data-toggle-theme], [data-toggle-theme-switch]').off().on('click',function(){
                ...
                $('body').toggleClass('theme-light theme-dark');
                    ...
                    if($('body').hasClass('theme-light')){
                        ...
                        localStorage.setItem('Apptastic-Theme','light-mode')
                    }
                    if($('body').hasClass('theme-dark')){
                        darkSwitch.prop('checked', true); 
                        localStorage.setItem('Apptastic-Theme','dark-mode')
                    }
                setTimeout(function(){allowTransitions();},150);
                return false;
            })
            if (localStorage.getItem('Apptastic-Theme') == "dark-mode") {darkSwitch.prop('checked', true); $('body').removeClass('theme-light').addClass('theme-dark');}
            if (localStorage.getItem('Apptastic-Theme') == "light-mode") {darkSwitch.prop('checked', false); $('body').removeClass('theme-dark').addClass('theme-light');}
        ...
        if($('.footer-bar-2, .footer-bar-4, .footer-bar-5').length){
            if(!$('.footer-bar-1 strong, .footer-bar-2 strong, .footer-bar-4 strong, .footer-bar-5 strong').length){
                $('.footer-bar-1 .active-nav, .footer-bar-2 .active-nav, .footer-bar-4 .active-nav, .footer-bar-5 .active-nav').append('<strong><u></u></strong>')
            }
        }        
        
        //Adding Local Storage for Visited Links
        localStorage.setItem('z_Apptastic-Visited:' + window.location.pathname, 'Page Visited');
        $.fn.visited = function (className) {
            className = className || false;
            $(this).each(function (index, element) {
                if (element.host == window.location.host &&
                    localStorage.getItem('z_Apptastic-Visited:' + element.pathname) ||
                    localStorage.getItem('z_Apptastic-Visited:' + element.pathname + '/')) {
                    if (className) {
        ...
        $('.list-group a').on('click',function(){if($(this).attr('href') != '#'){localStorage.setItem('z_Apptastic-Visited-Scroll-Position', $(window).scrollTop());}})
        if($('.check-visited').length){
            setTimeout(function(){
                var getScroll = localStorage.getItem('z_Apptastic-Visited-Scroll-Position');
                $(window).scrollTop(getScroll)
                 localStorage.removeItem('z_Apptastic-Visited-Scroll-Position');
            },200);
        }
        ...
        $('.header-large p').addClass('color-highlight').html(weekdayName[weekID.getDay()]  + ' ' +  dayName + daySuffix + ' ' + monthNames[monthID.getMonth()])
        ...
        $(window).on('scroll', function () {
            ...
            if (inside_header === true) {
                $('.header-scroll-effect').removeClass('header-transform');
                hide_back_to_top_badge();
                hide_scroll_ad();
            }
            else if(passed_header === true){
                $('.header-scroll-effect').addClass('header-transform');
...
//            }
        ...
        if(visitDetection.length){
            var lastVisitValue = localStorage.getItem('Apptastic-Last-Visited');
            var d = new Date();
            ...
            if(!lastVisitValue){
                $('.never-visited').show();
                $('.before-visited').hide();
                localStorage.setItem('Apptastic-Last-Visited', '<br>Your last visit was ' + strDate +' at '+ strTime)
            } else {
       ...
       $(".text-size-default").click(function() {$(".text-size-changer *").css("font-size", "");});
        ...
        function search_menu(){
            $('[data-search]').on('keyup', function() {
                ...
                $('.search-box a i').removeClass('disabled');
                if (searchVal != '') {
                ...
                if (searchVal == '') {
                    $('.search-box a i').addClass('disabled');
        ...
        setTimeout(function(){
            ...
            $('.text-slider').owlCarousel({loop:false, margin:0, nav:false, lazyLoad:true, items:3, autoplay: false, autoplayTimeout:4000});		
            $('.boxed-slider').owlCarousel({loop:false, margin:20, stagePadding:30, nav:false, lazyLoad:true, items:1, autoplay: false, autoplayTimeout:4000});		
            ...
            $('.double-slider').owlCarousel({loop: false, margin:20, stagePadding:30, nav: false, items: 3, autoplay: false, autoplayTimeout:6000});		
            ...
            $('.menulist-slider').owlCarousel({loop: false, margin:10, stagePadding:30, nav: false, items: 2, autoplay: false, autoplayTimeout:6000});
            ...
            $('.next-slide-user').on('click',function(){$('.owl-carousel').trigger('next.owl.carousel');});		
            $('.prev-slide-user').on('click',function(){$('.owl-carousel').trigger('prev.owl.carousel');});		
        },10);
        setTimeout(function(){
        ...
        function highlightLoader(){
            $('[data-highlight-style]').on('click',function(){
                ...
                localStorage.setItem('Apptastic-Highlight', getHighlight);
                $('#highlight').remove();
                $('head').append( $('<link rel="stylesheet" id="highlight" type="text/css" />').attr('href', getHighlight) );
                return false;
            });
            var rememberHighlight = localStorage.getItem('Apptastic-Highlight');
        ...
        if(pwaEnabled === true){
            ...
            var dismissDate = localStorage.getItem('Apptastic-PWA-Timeout-Value');
            if((day - dismissDate) > pwaRemind){
                localStorage.removeItem('Apptastic-PWA-Prompt')
            } 
                
            //Dismiss Prompt Button
            $('.pwa-dismiss').on('click',function(){
                console.log('User Closed Add to Home / PWA Prompt')
                localStorage.setItem('Apptastic-PWA-Prompt', 'install-rejected');
                $('body').find('#menu-install-pwa-android, #menu-install-pwa-ios, .menu-hider').removeClass('menu-active'); 
                localStorage.setItem('Apptastic-PWA-Timeout-Value', day);
            });
            ...
            if (isMobile.Android()) {
                function showInstallPrompt(){
                    if($('#menu-install-pwa-android, .add-to-home').length){
                        if (localStorage.getItem('Apptastic-PWA-Prompt') != "install-rejected") {
                            setTimeout(function(){
